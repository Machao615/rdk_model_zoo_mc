...
下载 `ultralytics/ultralytics` 仓库，并参考 Ultralytics 官方文档配置训练环境。
```bash
git clone https://github.com/ultralytics/ultralytics.git
```

模型训练请参考 Ultralytics 官方文档。源 `.pt` 权重需要从 `ultralytics/ultralytics` 仓库训练得到，或直接使用 Ultralytics 官方发布的预训练权重。请注意，训练阶段无需修改任何程序，也无需修改 `forward` 方法。
...
进入 Ultralytics 本地仓库，下载 Ultralytics 官方预训练权重，或使用自己按 Ultralytics 训练流程得到的 `.pt` 权重。这里以 YOLO11n-Detect 模型为例。
```bash
cd ultralytics
wget https://github.com/ultralytics/assets/releases/download/v8.3.0/yolo11n.pt
```

在 Ultralytics YOLO 的训练环境中运行本目录下的 `export_monkey_patch.py` 对模型进行导出。此步骤依赖 `ultralytics.YOLO`、PyTorch 和导出依赖，应在训练或导出环境中执行，不是在板端执行。 这个脚本会使用 `ultralytics.YOLO` 类加载 YOLO 的 `.pt` 模型，使用猴子补丁（Monkey Patch）的方法在 PyTorch 层面对模型进行替换，并调用 `ultralytics.YOLO.export` 方法导出。 导出的 ONNX 模型会保存在 `.pt` 模型同级目录下。
